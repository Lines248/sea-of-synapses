...
import BrainDiagram from "../Brain/BrainDiagram";
import type { SynapseNodeData } from "../../hooks/useNodeData";

interface HeaderProps {
  selectedNode: SynapseNodeData | null;
}

const Header: React.FC<HeaderProps> = ({ selectedNode }) => {
  return (
    <header className="relative flex items-center justify-between border-b border-slate-800/50 bg-gradient-to-b from-slate-950/95 via-slate-950/80 to-slate-950/60 px-6 py-5 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative w-11 h-11 flex items-center justify-center">
          <BrainDiagram selectedNode={selectedNode} />
        </div>

        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="absolute -inset-1 rounded-lg bg-gradient-to-r from-sky-500/20 to-cyan-500/20 blur opacity-60" />
      ...
      <div className="flex items-center gap-2 text-[10px] text-slate-500">
        <span className="rounded-full bg-slate-800/50 px-2 py-1">
          Neural Network
        </span>
      </div>
    </header>
